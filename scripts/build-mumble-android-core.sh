...
CORE_DYNSYMS="$("${LLVM_READELF}" --dyn-syms "${CORE_SO}")"
if ! printf '%s\n' "${CORE_DYNSYMS}" | grep -Eq 'GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+mainecho "Native DT_NEEDED entries before AAR deployment:"
...
; then
  ...
  printf '%s\n' "${CORE_DYNSYMS}" | grep -E 'main|JNI_OnLoad|NativeServer_' >&2 || true
  exit 4
fi
