...
if ! "${LLVM_READELF}" --dyn-syms "${CORE_SO}" | grep -Eq 'GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+mainecho "Native DT_NEEDED entries before AAR deployment:"
...
; then
  echo "ERROR: libvcserver does not export dynamic symbol main; Qt Android would crash when resolving the native entrypoint" >&2
